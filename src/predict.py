...
if os.path.exists(MODEL_SAVE_PATH):
    ...
    print(f"Error: Model file not found at {MODEL_SAVE_PATH}. Please train or download the model first.")
    exit()
...
class SpecificSentimentDataset(IMDBDataset):
    def __init__(self, base_data_dir, word2idx, sentiment_type):
        ...
        target_dir = os.path.join(base_data_dir, sentiment_type)
...
if __name__ == "__main__":
    ...
    print("\n--- Manual Text Prediction Test ---")
    test_texts = [
        # Texts
    ]

    for i, text_to_predict in enumerate(test_texts):
        sentiment, probability = predict_text(model, text_to_predict, words2idx, device)
        print(f"Test {i+1}: '{text_to_predict}'")
